...
unseen_data[1359, "MSSubClass"] <- 160
...
# training the mlr doing a 70/30 train/test split
library(caret)
set.seed(2187)
train.index <- createDataPartition(modeling_data$Neighborhood, p = 0.7, list = FALSE)
train <- modeling_data[train.index, ]
test <- modeling_data[-train.index, ]

mlr_model <- lm(LogSalePrice ~ ., data = train)

test_target <- test
test_target$LogSalePrice <- NULL
mlr_model_pred <- predict(mlr_model, test_target, interval = "prediction")

plot(mlr_model_pred[,1], test$LogSalePrice,
     main = "Multiple Linear Regression with\nAll Features",
     xlab = "Fitted Values",
     ylab = "True Values")
mlr_empty <- lm(LogSalePrice ~ 1, data = train)
scope = list(lower = formula(mlr_empty), upper = formula(mlr_model))

library(MASS)
backwardAIC = step(mlr_model, scope, direction = "backward", k = 2)



# anova(backwardAIC, mlr_model)

pred_back <- predict(backwardAIC, test_target, interval = "prediction")
plot(pred_back[,1], test$LogSalePrice,
     main = "Multiple Linear Regression with AIC\nSelected Features",
     xlab = "Predicted Values",
     ylab = "True Values")

red_model_R2_train <- summary(backwardAIC)$r.squared
full_model_R2_train <- summary(mlr_model)$r.squared

full_model_R2_test <- cor(mlr_model_pred[,1], test$LogSalePrice)^2
red_model_R2_test <- cor(pred_back[,1], test$LogSalePrice)^2

summ <- summary(backwardAIC)
best_model_formula <- summ[[1]]
best_form <- as.formula(best_model_formula)
best_mlr <- lm(best_form, data = modeling_data)
best_model_R2 <- summary(best_mlr)$r.squared
...
x = model.matrix(LogSalePrice ~ ., train)[, -1] #Dropping the intercept column.
...
train_features$LogSalePrice <- NULL
pred_ridge_train <- predict(best_ridge, s = best_lambda, newx = x)
actual_ridge_train <- train$LogSalePrice
preds_ridge_train <- pred_ridge_train

# calculating train r-squared
...
x_test <- model.matrix(LogSalePrice ~ ., test)[, -1]
preds_ridge_test <- predict(best_ridge, s = best_lambda, newx = x_test)
actual_ridge_test <- test$LogSalePrice
...
# adding fcator levels to unseen data to match model
...
actual_lasso_train <- train$LogSalePrice
...
actual_lasso_test <- test$LogSalePrice
...
actual_en_train <- train$LogSalePrice
...
actual_en_test <- test$LogSalePrice
...
oob.err = numeric(25)
for (mtry in 1:25) {
  fit = randomForest(LogSalePrice ~ ., data = train, mtry = mtry)
...
best_rf <- randomForest(LogSalePrice ~ ., train, importance = TRUE, mtry = best_mtry_rf)
# best_rf
# best_rf$importance
# importance(best_rf)
# varImpPlot(best_rf)
...
actual_rf_train <- train$LogSalePrice
...
pred_rf_test <- predict(best_rf, test, type = "response")
actual_rf_test <- test$LogSalePrice
...
boost_best <- gbm(LogSalePrice ~ ., data = train,
                  distribution = "gaussian",
                  n.trees = 2500,
                  interaction.depth = 5,
                  shrinkage = 0.1)
...
actual_boost_train <- train$LogSalePrice
...
preds_boost_test <- predict(boost_best, newdata = test, n.trees = 2500)
actual_boost_test <- test$LogSalePrice
...
lasso_coeffs <- data.frame("Feature" = row.names(coef(best_lasso)), "Coefficient" = coeffs)
